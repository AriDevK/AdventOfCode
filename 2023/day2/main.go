...
// define the max number of cubes for each color
const rCubes = 12
const gCubes = 13
const bCubes = 14
...
// partOne function will calculate the sum of all game ids that are possible to be played
func partOne() {
	var output int
	...
	for _, row := range rows {
		var isPossible bool
		rowDt := strings.Split(row, ":")
		gameDt := strings.Split(rowDt[0], " ")
		gameId, _ := strconv.Atoi(gameDt[1])

		dataset := rowDt[1]
		subsets := strings.Split(dataset, ";")
		...
		for _, subset := range subsets { //
			...
			data := strings.Split(subset, ",")
			for _, value := range data {
				cleanValue := strings.Trim(value, " ")
				valueDt := strings.Split(cleanValue, " ")
				qty, _ := strconv.Atoi(valueDt[0])
				class := valueDt[1]

				// increment the counter for each color
				if class == "red" {
					rCounter += qty
				} else if class == "green" {
					gCounter += qty
			...
			// check if the current counter is less than the max number of cubes for each color
			isPossible = rCounter <= rCubes && gCounter <= gCubes && bCounter <= bCubes
			if !isPossible {
				break
			}
		}

		// if the game is possible to be played, add the game id to the output
		if isPossible {
			output += gameId
		}
	}
...
// partTwo function will calculate the multiplication of the minimum required cubes for each color
func partTwo() {
	var output int
	rows := strings.Split(input, "\n")
	...
	for _, row := range rows {
		...
		dataset := strings.Split(row, ":")[1]
		normalizeDt := strings.Replace(dataset, ",", ";", -1)
		subsets := strings.Split(normalizeDt, ";")
		...
		for _, subset := range subsets { //
			...
			for _, value := range data {
				...
				// check if the current qty is greater than the current min required qty for each color
				if class == "red" && qty > rMinReq {
					rMinReq = qty
				}

				if class == "green" && qty > gMinReq {
					gMinReq = qty
				}

				if class == "blue" && qty > bMinReq {
					bMinReq = qty
				}
			}
		}
		output += rMinReq * gMinReq * bMinReq
	}
...
/***************************************************
	INPUT DATA DOWN HERE DO NOT MODIFY IT !!! :)
***************************************************/

var input = `Game 1: 5 red, 1 green; 6 red, 3 blue; 9 red; 1 blue, 1 green, 4 red; 1 green, 2 blue; 2 blue, 1 red
