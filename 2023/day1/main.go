...
// partOne get the first number on each line and the last number on each line, create a new number from them and add it to the output.
func partOne() {
	var output int
	data := strings.Split(input, "\n")

	for _, v := range data {
		cleanDt := removeLetters(v)
		numberDt, _ := strconv.Atoi(cleanDt[0] + cleanDt[len(cleanDt)-1])
		output += numberDt
	}
...
// partTwo same as partOne but we need to swap the name of the number with the value of the number.
func partTwo() {
	var output int
	data := strings.Split(input, "\n")

	for _, v := range data {
		splitDt := swapNameWithValue(v)
		cleanDt := removeLetters(splitDt)
		numberDt, _ := strconv.Atoi(cleanDt[0] + cleanDt[len(cleanDt)-1])
		output += numberDt
	}
